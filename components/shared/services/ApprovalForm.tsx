...
const ApprovalForm = ({ ordId }: ApprovalFormProps) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const data = await UpdateAndApplyforApprovalAction({
      org: {
        orgHq: values.orgHq,
    ...
    <div className="w-full min-h-screen md:px-32 px-2 bg-zinc-100">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-full">
                  <FormLabel>Organization Website {ordId} </FormLabel>
                  <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full mt-10">
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="please write your company details"
                        {...field}
                      />
                    </FormControl>
                    <FormDescription>
          ...
          {/* third row */}
          <Button className="w-full bg-zinc-800" type="submit">
            Submit
          </Button>
        </form>
      </Form>
